...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then(function(data) {
    ...
    var metadata = data.metadata;
    var metaFilteredSamples = metadata.filter(metaObj => metaObj.id == sample);

    // 2. Create a variable that holds the first sample in the metadata array.
    var metaFilteredSample = metaFilteredSamples[0];

    // 3. Create a variable that holds the washing frequency.
    var wash = metaFilteredSample.wfreq;
    ...
    var gaugeData = [{
      ...
      gauge: {
        axis: { range: [null, 10], dtick: 2, tickwidth: 1, tickcolor: "grey" },
        bar: { color: "black" },
        steps: [
          { range: [0, 2], color: "red" },
